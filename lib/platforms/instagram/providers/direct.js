    ...
    if (!result) {
      try {
        ...
        for (const item of items) {
          if (item.type === 'video') {
             formats.push({
               id: `ig_video_${videoCounter}`,
               quality: `Video ${videoCounter}`,
               extension: "mp4",
               hasVideo: true,
             ...
             formats.push({
               id: `ig_image_${imageCounter}`,
               quality: `Image ${imageCounter}`,
               extension: /^[a-z0-9]{2,5}$/i.test(ext) ? ext.toLowerCase() : "jpg",
               hasVideo: false,
               hasAudio: false,
               url: item.url,
               thumbnail: item.url
        ...
        if (formats.length === 0) {
           throw new InstagramAccessibilityError("No downloadable media found in this Instagram post.");
        }

        result = {
          id: `ig_${mediaId}`,
          platform: "instagram",
          type: formats.some(f => f.hasVideo) ? "video" : "image",
