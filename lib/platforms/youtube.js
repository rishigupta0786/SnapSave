...
export async function analyzeYoutubeMedia(url) {
  try {
    ...
    // Audio-only option: prefer a dedicated audio-only stream, but fall back to
    // extracting audio from the best muxed video+audio stream (e.g. itag 18)
    // when no separate audio-only stream is available, such as under SABR restrictions.
    const audioFormats = info.formats.filter(f =>
      f.url && f.vcodec === 'none' && f.acodec && f.acodec !== 'none'
    );
    const audioSourceId = audioFormats.length > 0
      ? audioFormats[0].format_id
      : videoFormats.find(f => f.acodec && f.acodec !== 'none')?.format_id;

    if (audioSourceId) {
       formats.push({
          id: `mp3:${audioSourceId}`,
          quality: "Best Audio Only (MP3)",
          extension: "mp3",
          hasVideo: false,
          hasAudio: true
       });
...
export function downloadYoutubeMedia(url, formatId, destination, onProgress) {
  return new Promise((resolve, reject) => {
    const isMp3 = formatId.startsWith("mp3:");
    const realFormatId = isMp3 ? formatId.slice(4) : formatId;
